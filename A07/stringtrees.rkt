...
(define (list-of-stringtree-insert name insertion-path los)
  (cond [(empty? insertion-path)
        ...
        [(= (first insertion-path) 0)
         (cons (stringtree-insert name (rest insertion-path) (first los))
               (rest los))]
        [else
         (cons (first los)
                ...
                (("Tokido" ())
                 ...
                 ("Smug" (("Fuudo" ())
                          ("Mena" (("Caba" ())
                                   ...
                                   ("NuckleDu" ())))
                          ("SnakeEyez" ()))))))
(check-expect (stringtree-insert "Daigo" (list 2 1) celebs)
              '("Punk"
                (("Tokido" ())
                 ("Marn" (("JWong" ()) ("Ricki" ()) ("KBrad" ())))
                 ("Smug" (("Fuudo" ())
...
(check-expect (stringtree-insert "Jake" (list 0 0 0 0) small-example-tree)
              '("Lucas" (("Kevin" (("Yolanda" (("Pedram" (("Jake" ()))))))))))
(check-expect (stringtree-insert "Jake" (list 0 0 1) small-example-tree)
              '("Lucas" (("Kevin" (("Yolanda" (("Pedram" ()) ("Jake" ()))))))))
(check-expect (stringtree-insert "Jake" (list 0) small-example-tree)
              '("Lucas" (("Jake" ()) ("Kevin" (("Yolanda" (("Pedram" ()))))))))
(check-error (stringtree-insert "Jake" (list 0 0 2) small-example-tree)
             "Invalid insert")
(check-error (stringtree-insert "Jake" (list 0 0 0 0 0) small-example-tree)
             "Invalid insert")
